...
async function loadBunnyChefContract(App, tokens, prices, chef, chefAddress, chefAbi,
    ...
    const poolInfos = await Promise.all(Vaults.map(a => loadBunnyPoolInfo(App, tokens, prices, a)));
    let tvl = 0, userTvl = 0
    for(const p of poolInfos.filter(p => p))
    {
        printBunnyContract(p);
...
async function loadBunnyPoolInfo(App, tokens, prices, contractAddress) {
    try {
        const contract = await new ethers.Contract(contractAddress, BUNNY_STAKING_ABI, App.provider);
...
async function printBunnyContract(poolInfo) {
